...
export default function InstmanegementListPage({ orgId }) {
  ...
  const handleEditRow = (id) => {
    navigate(PATH_DASHBOARD.orgmanagment.editstaff(id));
  };

  const handleChangePassword = (id) => {
    navigate(PATH_DASHBOARD.orgmanagment.changepassword(id));
  }
...
function applyFilter({ inputData, comparator, filterName }) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (item) => item?.name.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 || item?.email_id.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
